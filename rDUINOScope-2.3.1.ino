...
float OBSERVATION_LONGITUDE = 174.7773; // (23.3333* - Home)
float OBSERVATION_LATTITUDE = -41.3216; // (42.6378* - Home)
float OBSERVATION_ALTITUDE = 35; // Wellingon, NZ
int TIME_ZONE = 12;
// .............................................................
...
//  touch.begin();
  myTouch.InitTouch(PORTRAIT); //txs
  //myTouch.InitTouch(LANDSCAPE);//txs
...
//  Timer3.start(Clock_Sidereal); // executes the code every 62.329 ms.
  ...
  if (analogRead(A6) < 800){
    IS_NIGHTMODE = true; //txs REVERSED changed from true to false to avoid having to have a nightmode switch 
    texts = Maroon;
    l_text = RED;
    d_text = Maroon;
    title_bg = RED;
...
//          Serial.print(items);
  ...
  delay(1000);
  Serial.println("initialization of GPS running");
 ...
 if ((DELAY_Slew + 1 <= millis()) && (IS_OBJ_FOUND == false)){
  ...
  if (IS_OBJ_FOUND == true){    
      ...
      if (myTouch.dataAvailable() == true){  
          myTouch.read();
          tx =  0.6 * myTouch.getX();//txs added multiplier
          ty  = 1.6666666666 * myTouch.getY();//txs added multiplier
       }
      considerTouchInput(tx, ty);

      
     Serial.print("X:");//txs added
     Serial.print(tx);//txs added
     Serial.print(" Y:");//txs added
     Serial.println(ty);//txs added
...
//...
    ...
    SLEW_DEC_microsteps = DEC_90 + (DEC_decimal * DEC_D_CONST);    // Hardware specific code // Following advice from Fernando Haller here, the '-' had to become '+' sign for the Southern Hemisphere: https://www.facebook.com/groups/150875201997704/?multi_permalinks=925638177854732%2C922993624785854&notif_id=1588099205796770&notif_t=group_activity
